...
from app.crud.subject_chat_crud import (create_subject_chat_message,
                                        create_subject_chat_answer,
                                        create_subject_attach_file_db,
                                        create_subject_recipient_db,
                                        get_last_messages_for_subject_chat_db,
                                        get_messages_for_subject_chat_by_pagination_db,
                                        select_message_by_id_db,
                                        select_recipient_by_message_id,
                                        select_last_message_db,
                                        select_last_answer_db,
                                        update_read_by_for_message_db,
                                        update_read_by_for_answer_db)
...
from app.utils.save_images import save_subject_chat_file, delete_subject_chat_file
from app.utils.subject_chat import (set_subject_chat_last_messages_dict,
                                    set_subject_chat_last_answer_dict,
                                    set_subject_chat_last_message_dict)
...
async def delete_file_from_chat(
    ...
    if user.student or user.teacher or user.moder:
        return delete_subject_chat_file(file_path=file_path)
    return HTTPException(status_code="403", detail="Teacher can't use group chat")
